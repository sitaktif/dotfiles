...
`dotspacemacs/user-config' first."
  ...
  ;; Stop complaining about .bashrc containing PATH/MANPATH instead of
  ;; .bash_profile since I'm sourcing one from the other
...
you should place your code here."
  ...
  ;; TAB to "preview" a selection (TAB again to go back). C-z to do an action on it.
  ;; C-c ? to get help. C-t to toggle vert/horiz helm buffer.

  ;;
  ;; SETTINGS
  ;;

  "Enable flycheck (linter) by default"
  ...
  ;; This doesn't work? (modify-syntax-entry ?_ "w")

  ;; Vim settings

  ;; TODO
  ;; Fix issues:
  ;; - when visually selecting then indenting with '>', the selection stays
  ;; - when visually selecting then indenting with '>', then pressing escape, the '.' operator doesn't work
  ;; - the ex command line last command is displayed rather than a bare ':'
  ;; - the ex command line repeats the last command if typing ':<cr>' or ':<bs><cr>' (should be a noop)

  ;; "word" includes underscores in Vim
  (defadvice evil-forward-word-begin (around underscore-is-forward-word-begin activate)
    (let ((table (copy-syntax-table (syntax-table))))
      (modify-syntax-entry ?_ "w" table)
      (with-syntax-table table
        ad-do-it)))
  (defadvice evil-forward-word-end (around underscore-is-forward-word-end activate)
    (let ((table (copy-syntax-table (syntax-table))))
      (modify-syntax-entry ?_ "w" table)
      (with-syntax-table table
        ad-do-it)))
  (defadvice evil-backward-word-begin (around underscore-is-backward-word-begin activate)
    (let ((table (copy-syntax-table (syntax-table))))
      (modify-syntax-entry ?_ "w" table)
      (with-syntax-table table
        ad-do-it)))
  (defadvice evil-backward-word-end (around underscore-is-backward-word-end activate)
    (let ((table (copy-syntax-table (syntax-table))))
      (modify-syntax-entry ?_ "w" table)
      (with-syntax-table table
        ad-do-it)))
  (defadvice evil-a-word (around underscore-is-a-word activate)
    (let ((table (copy-syntax-table (syntax-table))))
      (modify-syntax-entry ?_ "w" table)
      (with-syntax-table table
        ad-do-it)))
  (defadvice evil-inner-word (around underscore-is-inner-word activate)
    (let ((table (copy-syntax-table (syntax-table))))
      (modify-syntax-entry ?_ "w" table)
      (with-syntax-table table
        ad-do-it)))
  ...
  ;; Cmd-alt-left and cmd-alt-right to move between windows
  (global-set-key (kbd "s-w") 'delete-window)

  ;; Cmd-alt-left and cmd-alt-right to move between windows
  (global-set-key (kbd "M-s-ˍ") 'previous-multiframe-window)
  (global-set-key (kbd "M-s--") 'next-multiframe-window)

  ;; Cmd-alt-up and cmd-alt-down for linter errors
  (global-set-key (kbd "M-s-˚") 'flycheck-previous-error)
  (global-set-key (kbd "M-s-˝") 'flycheck-next-error)

  ;; Cmd-plus and cmd-minus to change text size
  (global-set-key (kbd "s-=") 'text-scale-increase)
  (global-set-key (kbd "s--") 'text-scale-decrease)

  ;; VIM bindings

  ;; New Vim surround plugin default is to use "S" for surround and "s" for edit
