...
-- Battery widget
local sita_battery = require("sita_battery")
...
xrandr_external_top = "xrandr --output DP3 --primary --mode 2560x1440 --output LVDS1 --mode 1366x768 --below DP3"
...
-- Table stuff (does not exist in the fucking standard library??)
...
sita_batterywidget = wibox.widget.textbox("batterywidget")
sita_batterywidget:set_markup("Bat: <span color='#333333'>(init)</span>")
sita_batterytimer = timer({ timeout = 3, started = true } )
sita_batterytimer:connect_signal("timeout", function()
    sita_battery.batteryInfo(sita_batterywidget, "BAT0")
end)
sita_batterytimer:start()

-- Monitor the screens/clients
-- sita_monitortimer = timer({ timeout = 2, started = true } )
-- sita_monitortimer:connect_signal("timeout", function()
--     notify(table.tostring(client.get()))
-- end)
-- sita_monitortimer:start()
...
for s = 1, screen.count() do
    ...
    right_layout:add(sita_batterywidget)
...
-- If I keep these here, they will be relaunched each time..
--awful.util.spawn("firefox")
--awful.util.spawn("gnome-terminal --role=kollok -x ssh kollok.org")
...
--         local cal = awful.util.pread("cal -m " .. 1)
