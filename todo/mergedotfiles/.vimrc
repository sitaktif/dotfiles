...
set statusline=%F%m%r%h%w\ %=\ [L=%3l/%L,C=%2v/%2{len(getline('.'))}]\ [%p%%]
"set statusline=%F%m%r%h%w\ \%{&ff}\ %Y\ [%03.3b\ %02.2B]\ [POS=%04l/%L,%04v/%{len(getline('.'))}]\ [%p%%]

"set ruler "Show line,col in statusbar (overriden by statusline)
...
" and use arrow keys only in some context
noremap <up> 10<c-y>
noremap <down> 10<c-e>
noremap <left> 10zh
noremap <right> 10zl
...
" AMADEUS
let g:AMA_USER_TEAM = 'stg'

function! AMA_RGR_commit()
    normal 10GANone
    normal jddARegression fixing
endfunc

map \ar :%s/UCI+\zs\w\{14}\ze+/{\[^+]}/g<cr>

"~/.vim,/data/softs/vim-7.2/share/vim/vimfiles,/data/softs/vim-7.2/share/vim/vim72,/data/softs/vim-7.2/share/vim/vimfiles/after,~/.vim/after
"helptags /data/softs/vim-7.2/share/vim/vim72/doc
"let g:AMA_CM_TEAM_PACKAGES=1
